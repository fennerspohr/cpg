<template>
  <header class="bg-base-100 w-full select-none flex flex-col shadow-sm border-b border-base-300">
    ...
    <div class="flex items-center justify-between p-2 border-t-2 border-t-white">
      ...
      <div class="flex items-center gap-3 pr-4">
        <label class="text-[10px] font-bold uppercase text-win-text">Busca Rápida:</label>
        <div class="flex items-center gap-1">
          <div class="relative flex items-center">
            <input
              ...
              class="h-7 w-48 pl-7 pr-2 text-xs bg-white border-2 border-t-base-300 border-l-base-300 border-r-white border-b-white outline-none focus:bg-win-focus placeholder:italic"
            />
            <Icon name="lucide:search" class="absolute left-2 text-win-dim text-sm pointer-events-none" />
          </div>
          <button type="button" @click="handleQuickSearch"
            class="h-7 px-2.5 border-2 border-t-white border-l-white border-r-base-300 border-b-base-300 bg-base-100 text-[10px] font-bold uppercase flex items-center gap-1 hover:brightness-110 active:border-t-base-300 active:border-l-base-300 active:border-r-white active:border-b-white">
  ...
  <Teleport to="body">
    <div v-if="ajudaAberta" class="fixed inset-0 z-200 flex items-center justify-center bg-black/30"
      ...
      <div class="w-[460px] bg-base-100 border-2 border-t-white border-l-white border-r-win-text border-b-win-text shadow-[2px_2px_0_#000]">

        <div class="flex items-center justify-between bg-win-navy px-2 py-1 mx-0.5 mt-0.5">
          <span class="text-white text-xs font-bold flex items-center gap-1.5">
            <Icon name="lucide:circle-help" class="text-sm" />
            Ajuda — Sistema CPG
          </span>
          <button type="button" @click="ajudaAberta = false"
            class="bg-base-100 border-2 border-t-white border-l-white border-r-base-300 border-b-base-300 w-5 h-5 flex items-center justify-center text-xs font-bold hover:brightness-110 active:border-t-base-300 active:border-l-base-300 active:border-r-white active:border-b-white">
            ×
          </button>
        </div>
        ...
        <div class="p-4 flex flex-col gap-3 text-xs">
          <div v-for="grupo in AJUDA" :key="grupo.titulo"
            ...
            <p class="text-[9px] font-bold uppercase text-win-text mb-1.5 border-b border-base-300 pb-0.5">{{ grupo.titulo }}</p>
            <div class="flex flex-col gap-1">
              <div v-for="item in grupo.itens" :key="item.acao" class="flex gap-2">
                ...
                <span class="text-win-text">{{ item.desc }}</span>
              </div>
            </div>
          </div>
        </div>
...
const emit = defineEmits<{ 'open-cities': [] }>()
