<template>
  <div class="h-full p-6 bg-[#c0c0c0] overflow-auto font-sans">
    <div class="max-w-3xl mx-auto bg-base-100 border-2 border-t-white border-l-white border-r-base-300 border-b-base-300 shadow-md mb-10">
      ...
      <div class="bg-primary px-2 py-1 flex items-center justify-between mx-0.5 mt-0.5">
        ...
        <button @click="$router.back()" class="bg-base-100 border border-t-white border-l-white border-r-black border-b-black w-4 h-4 flex items-center justify-center text-xs pb-0.5 leading-none shadow-sm hover:bg-[#e0e0e0] active:border-t-black active:border-l-black active:border-r-white active:border-b-white">
          ×
        </button>
      </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="flex flex-col gap-1">
              ...
              <input v-model="form.nome" type="text" required 
                class="bg-white border-2 border-t-base-300 border-l-base-300 border-r-white border-b-white p-1 text-sm outline-none focus:bg-[#ffffcc]" />
            </div>
            <div class="flex flex-col gap-1">
  ...
  <fieldset class="border-2 border-t-white border-l-white border-r-base-300 border-b-base-300 p-3">
    <legend class="text-[10px] font-bold px-1 uppercase text-[#404040]">Nascimento</legend>
    <div class="space-y-3">
      <input v-model="form.datanasc" type="date" 
        class="bg-white border-2 border-t-base-300 border-l-base-300 border-r-white border-b-white p-1 text-sm outline-none w-full" />
      
      <div class="flex gap-2 items-center">
        <select v-model="form.localnasc" class="bg-white border-2 border-t-base-300 border-l-base-300 border-r-white border-b-white p-1 text-sm outline-none w-full max-w-[78%] focus:bg-[#ffffcc]">
        ...
        <button type="button" @click="modalLocalAberto = true" 
          class="border-2 border-t-white border-l-white border-r-base-300 border-b-base-300 bg-base-100 px-2 py-1.5 w-1/5 text-[9px] font-bold hover:bg-[#e0e0e0] active:border-t-base-300 active:border-r-white text-center leading-none"
  ...
  <fieldset class="border-2 border-t-white border-l-white border-r-base-300 border-b-base-300 p-3">
    ...
    <div class="space-y-3">
      ...
      <div class="flex gap-2 items-center">
        <select v-model="form.localbatismo" class="bg-white border-2 border-t-base-300 border-l-base-300 border-r-white border-b-white p-1 text-sm outline-none w-full max-w-[78%] focus:bg-[#ffffcc]">
          <option :value="null">Selecione o local...</option>
          <option v-for="loc in locais" :key="loc.id" :value="loc.id">
            {{ loc.descricao }} <span v-if="loc.estado">- {{ loc.estado }}</span>
          </option>
        ...
        <button type="button" @click="modalLocalAberto = true" 
          class="border-2 border-t-white border-l-white border-r-base-300 border-b-base-300 bg-base-100 px-2 py-2.5 w-1/5 text-[9px] font-bold hover:bg-[#e0e0e0] active:border-t-base-300 active:border-r-white text-center leading-none"
          title="Cadastrar nova cidade">
          Novo
        </button>
      </div>
  ...
  <fieldset class="border-2 border-t-white border-l-white border-r-base-300 border-b-base-300 p-3">
    ...
    <div class="space-y-3">
      ...
      <div class="flex gap-2 items-center">
        <select v-model="form.localmorte" class="bg-white border-2 border-t-base-300 border-l-base-300 border-r-white border-b-white p-1 text-sm outline-none w-full max-w-[78%] focus:bg-[#ffffcc]">
          <option :value="null">Selecione o local...</option>
          <option v-for="loc in locais" :key="loc.id" :value="loc.id">
            {{ loc.descricao }} <span v-if="loc.estado">- {{ loc.estado }}</span>
          </option>
        ...
        <div class="flex flex-col gap-1 px-1">
          ...
          <textarea v-model="form.obs" 
            class="bg-white border-2 border-t-base-300 border-l-base-300 border-r-white border-b-white p-1 text-sm outline-none h-24 resize-none focus:bg-[#ffffcc]"></textarea>
        </div>
        ...
        <div class="flex justify-end gap-3 pt-4 border-t border-base-300">
          ...
          <button type="submit" :disabled="loading"
            class="border-2 border-t-white border-l-white border-r-base-300 border-b-base-300 active:border-t-base-300 active:border-l-base-300 active:border-r-white active:border-b-white bg-base-100 text-xs py-1 px-10 font-bold disabled:opacity-50">
