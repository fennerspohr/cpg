<template>
  <div class="flex h-full bg-[#c0c0c0] font-sans overflow-hidden">
    ...
    <div class="flex flex-col flex-1 min-w-0 overflow-hidden">
      ...
      <!-- Barra de título -->
      <div class="flex-none flex items-center justify-between bg-primary border-2 border-t-white border-l-white border-r-[#808080] border-b-[#808080] px-3 py-1 m-2 mb-1">
        ...
        <button
          ...
          class="flex items-center gap-1 border-2 bg-[#d4d0c8] px-2 py-0.5 text-[9px] font-bold uppercase hover:brightness-110"
      ...
      <div class="flex-none grid grid-cols-2 gap-2 px-2 pb-1">
        ...
        <!-- Pessoa 1 -->
        <fieldset class="border-2 border-t-white border-l-white border-r-[#808080] border-b-[#808080] px-3 pt-1 pb-3 relative">
          <legend class="text-[10px] font-bold px-1 uppercase text-[#404040]">Pessoa 1</legend>
          <div v-if="pessoa1"
            class="border-2 border-t-white border-l-white border-r-[#808080] border-b-[#808080] px-3 py-2 flex items-center justify-between"
            ...
            <button @click="limparPessoa1"
              class="border-2 border-t-white border-l-white border-r-[#808080] border-b-[#808080] bg-[#d4d0c8] w-6 h-6 flex items-center justify-center text-xs font-bold hover:brightness-110 ml-2 flex-none">
              ×
            </button>
          </div>
          <div v-else class="relative">
            <input v-model="busca1" type="text" placeholder="Digite o nome para buscar..."
              @focus="foco1 = true" @blur="setTimeout(() => foco1 = false, 150)"
              class="bg-white border-2 border-t-[#808080] border-l-[#808080] border-r-white border-b-white px-2 py-1.5 text-sm outline-none focus:bg-[#ffffcc] w-full" />
            <div v-if="foco1 && resultados1.length"
              class="absolute top-full left-0 right-0 z-30 border-2 border-t-[#808080] border-l-[#808080] border-r-white border-b-white bg-white max-h-52 overflow-y-auto shadow-md">
            ...
            <p v-else-if="foco1 && busca1 && !resultados1.length"
              class="absolute top-full left-0 right-0 z-30 bg-white border-2 border-t-[#808080] border-l-[#808080] border-r-white border-b-white px-3 py-2 text-[10px] text-[#808080] italic">
              Nenhum resultado.
            </p>
          </div>
        </fieldset>
        ...
        <!-- Pessoa 2 -->
        <fieldset class="border-2 border-t-white border-l-white border-r-[#808080] border-b-[#808080] px-3 pt-1 pb-3 relative">
          <legend class="text-[10px] font-bold px-1 uppercase text-[#404040]">Pessoa 2</legend>
          <div v-if="pessoa2"
            class="border-2 border-t-white border-l-white border-r-[#808080] border-b-[#808080] px-3 py-2 flex items-center justify-between"
          ...
          <div v-else class="relative">
            <input v-model="busca2" type="text" placeholder="Digite o nome para buscar..."
              @focus="foco2 = true" @blur="setTimeout(() => foco2 = false, 150)"
              class="bg-white border-2 border-t-[#808080] border-l-[#808080] border-r-white border-b-white px-2 py-1.5 text-sm outline-none focus:bg-[#ffffcc] w-full" />
      ...
      <div class="flex-none flex justify-center pb-1">
        <button @click="buscarRelacao" :disabled="!pessoa1 || !pessoa2 || buscando"
          class="border-2 border-t-white border-l-white border-r-[#808080] border-b-[#808080] bg-[#d4d0c8] px-10 py-1.5 text-xs font-bold uppercase hover:brightness-110 disabled:opacity-40 flex items-center gap-2 active:border-t-[#808080] active:border-l-[#808080] active:border-r-white active:border-b-white">
      ...
      <!-- Área de resultado -->
      <div class="flex-1 mx-2 mb-2 border-2 border-t-[#808080] border-l-[#808080] border-r-white border-b-white overflow-hidden flex flex-col bg-[#d4d0c8]">
        <div v-if="!resultado && !buscando && !status"
          class="flex-1 flex flex-col items-center justify-center gap-3 text-[#808080]">
        ...
        <div v-else-if="status?.tipo === 'erro'"
          ...
          <p class="text-sm text-[#606060] text-center">
        ...
        <template v-else-if="resultado?.length">
          <div class="flex-none border-b-2 border-[#808080] px-4 py-1.5 bg-[#c8c4bc] flex items-center gap-2">
      ...
      <div class="flex-none mx-2 mb-1 flex justify-between items-center">
        ...
        <div v-if="status" class="border-2 border-t-[#808080] border-l-[#808080] border-r-white border-b-white px-2 py-0.5">
    ...
    <transition name="slide">
      <div v-if="painelAberto" class="flex flex-col w-64 flex-none border-l-2 border-[#808080] bg-[#d4d0c8] overflow-hidden">
        ...
        <div class="flex-none flex items-center justify-between bg-primary px-2 py-1">
          ...
          <button @click="painelAberto = false"
            class="bg-[#d4d0c8] border border-t-white border-l-white border-r-[#808080] border-b-[#808080] w-5 h-5 flex items-center justify-center text-sm font-bold">×</button>
        </div>
        ...
        <!-- Busca no painel -->
        <div class="flex-none p-1.5 border-b border-[#808080]">
          <input v-model="buscaLateral" type="text" placeholder="Buscar por nome..."
            class="w-full bg-white border-2 border-t-[#808080] border-l-[#808080] border-r-white border-b-white px-2 py-1 text-xs outline-none" />
        </div>

        <!-- Lista -->
        <div class="flex-1 overflow-y-auto m-1 bg-white border-2 border-t-[#808080] border-l-[#808080] border-r-white border-b-white">
          <table class="w-full text-left border-collapse">
            <thead class="sticky top-0 bg-[#d4d0c8]">
              <tr>
                <th class="px-1 py-0.5 text-[8px] font-bold uppercase border-b border-[#808080] w-8">ID</th>
                <th class="px-1 py-0.5 text-[8px] font-bold uppercase border-b border-[#808080]">Nome</th>
                <th class="px-1 py-0.5 text-[8px] font-bold uppercase border-b border-[#808080] w-10 text-right">Nasc.</th>
              </tr>
            </thead>
            <tbody>
        ...
        <div class="flex-none border-t border-[#808080] px-2 py-1 flex flex-col gap-0.5">
          <div class="flex items-center gap-1 text-[9px]">
            <span class="font-bold uppercase text-[#404040] w-14">Pessoa 1:</span>
            ...
            <span v-else class="text-[#808080] italic">não selecionada</span>
          </div>
          <div class="flex items-center gap-1 text-[9px]">
            <span class="font-bold uppercase text-[#404040] w-14">Pessoa 2:</span>
          ...
          <span class="text-[8px] text-[#808080]">{{ pessoasFiltradas.length }} registro(s)</span>
        </div>
