<template>
  <div class="w-full h-full bg-background flex flex-1 items-center justify-center overflow-hidden relative">
    ...
    <div v-if="!pessoaAtual"
      class="bg-[#d4d0c8] border-2 border-t-white border-l-white border-r-[#404040] border-b-[#404040] shadow-[2px_2px_0_#000] p-8 text-center">
      <Icon name="lucide:alert-circle" class="text-4xl text-red-700 mb-3" />
      <p class="text-sm font-bold uppercase text-[#404040]">Pessoa não encontrada (ID {{ pessoaId }})</p>
      <button @click="$router.push('/')"
        class="mt-4 border-2 border-t-white border-l-white border-r-[#808080] border-b-[#808080] bg-[#d4d0c8] text-[10px] font-bold uppercase py-1 px-4 hover:brightness-110">
        Voltar à Lista
      </button>
    </div>

    <div v-else class="bg-[#d4d0c8] border-2 border-t-white border-l-white border-r-[#404040] border-b-[#404040] shadow-[2px_2px_0_#000] flex"
      style="width: 860px; height: 560px">

      <div class="flex flex-col flex-1 min-w-0 overflow-hidden">

        <!-- barra de título -->
        <div class="flex-none flex items-center justify-between bg-[#7b3f00] px-2 py-1 mx-0.5 mt-0.5">
          <span class="text-white text-xs font-bold flex items-center gap-1.5">
            <Icon name="lucide:pencil" class="text-sm" />
            Editar Ficha de Pessoa
            <span class="opacity-75 font-normal">— {{ pessoaAtual.nome }} {{ pessoaAtual.sobrenome }}</span>
          </span>
        ...
        <!-- badge de edição -->
        <div class="flex-none bg-[#fff3cd] border-b border-[#f0c040] px-3 py-1 flex items-center gap-1.5">
          <Icon name="lucide:pencil" class="text-[10px] text-[#7b3f00]" />
          <span class="text-[10px] font-bold text-[#7b3f00] uppercase tracking-wide">
            Modo Edição — ID {{ String(pessoaId).padStart(3, '0') }}
          </span>
          <span class="text-[10px] text-[#7b3f00] ml-1">
            (alterações nos vínculos substituem os anteriores)
          </span>
        </div>
              ...
              <div class="flex-1 overflow-y-auto flex flex-col gap-1.5 min-h-0">
                ...
                <div v-for="(rel, index) in form.relacoes" :key="index"
                  ...
                  <div class="flex justify-between items-center bg-[#b8b4ac] px-1.5 py-1 border border-[#808080] mb-0.5">
                    <span class="text-[9px] font-bold uppercase text-[#303030]">
                      {{ rel.id ? 'Vínculo existente' : 'Novo vínculo' }}
                    </span>
                    <button type="button"
          ...
          <div class="flex-none flex justify-between items-center pt-1 border-t border-[#808080]">
            ...
            <div class="flex gap-1.5">
              ...
              <button type="submit" :disabled="loading"
                class="border-2 border-t-white border-l-white border-r-[#808080] border-b-[#808080] bg-[#d4d0c8] text-[10px] font-bold uppercase py-1 px-5 hover:brightness-110 disabled:opacity-50 active:border-t-[#808080] active:border-l-[#808080] active:border-r-white active:border-b-white flex items-center gap-1">
    ...
    <!-- painel lateral de pessoas cadastradas -->
    <transition name="slide-screen">
      <div v-if="painelAberto && pessoaAtual"
        ...
        <div class="flex-none p-2 border-b border-[#808080]">
          <div class="text-[10px] text-gray-700 font-bold mb-1 uppercase">Filtre e clique para vincular rápido!</div>
...
const { data: pessoaData                                 } = await useFetch<any[]>(`/api/pessoa?nome=${pessoaId}`)
const { data: relacoesData                               } = await useFetch<any[]>(`/api/relacao?id=${pessoaId}`)
...
// Re-inicializa o form se os dados chegarem depois (navegação client-side)
...
async function handleSubmit() {
  loading.value = true
  try {
    // 1. Cria pessoas novas (modo rápido)
    ...
    alert('Erro ao salvar: ' + (err.data?.message || err.message || 'Verifique os dados.'))
  } finally {
    loading.value = false
  }
}
