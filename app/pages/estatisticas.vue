<template>
  <div class="h-full flex flex-col font-sans bg-[#c0c0c0] overflow-hidden">
    ...
    <div class="flex-none flex items-center justify-between bg-primary border-2 border-t-white border-l-white border-r-base-300 border-b-base-300 px-3 py-1 m-2 mb-0">
      ...
      <button @click="refresh()" title="Atualizar"
    ...
    <div v-if="pending" class="flex-1 flex items-center justify-center">
      <p class="text-xs uppercase animate-pulse text-[#404040] tracking-widest">Carregando estatísticas...</p>
    </div>
      ...
      <div class="flex-1 overflow-y-auto p-2 flex flex-col gap-2">
        ...
        <div class="grid grid-cols-5 gap-2">
          <div v-for="c in cards" :key="c.label"
            ...
            <div class="flex items-center gap-1.5">
              ...
              <span class="text-[8px] font-bold uppercase text-[#505050] tracking-wide leading-tight">{{ c.label }}</span>
            </div>
            <p class="text-3xl font-bold text-black leading-none mt-0.5">{{ c.valor }}</p>
            <p class="text-[9px] text-[#707070]">{{ c.sub }}</p>
          </div>
        </div>
        ...
        <div class="grid grid-cols-3 gap-2">
          ...
          <fieldset class="border-2 border-t-white border-l-white border-r-base-300 border-b-base-300 bg-base-100 px-3 pt-1 pb-3 flex flex-col gap-2">
            <legend class="text-[9px] font-bold px-1 uppercase text-[#404040]">Distribuição por Sexo</legend>
          ...
          <div class="flex flex-col gap-2">
            <fieldset class="border-2 border-t-white border-l-white border-r-base-300 border-b-base-300 bg-base-100 px-3 pt-1 pb-2">
              <legend class="text-[9px] font-bold px-1 uppercase text-[#404040]">Cobertura de Dados</legend>
              <div class="flex flex-col gap-1 mt-1">
                <div v-for="cv in coberturaFmt" :key="cv.label" class="flex items-center gap-2">
                  <span class="text-[9px] text-[#505050] w-20 shrink-0">{{ cv.label }}</span>
                  ...
                  <span class="text-[9px] font-mono text-[#404040] w-8 text-right">{{ cv.pct }}%</span>
                </div>
              </div>
            </fieldset>
            <fieldset class="border-2 border-t-white border-l-white border-r-base-300 border-b-base-300 bg-base-100 px-3 pt-1 pb-2 flex flex-col gap-2">
              <legend class="text-[9px] font-bold px-1 uppercase text-[#404040]">Recordes</legend>
              <div v-if="d.maisAntiga" class="flex flex-col">
                <span class="text-[8px] font-bold uppercase text-[#505050] flex items-center gap-1">
                ...
                <p class="text-[9px] font-mono text-[#606060]">{{ formatarData((d.maisAntiga as any).datanasc) }}</p>
              </div>
              <div v-if="d.maisRecente" class="flex flex-col border-t border-base-300 pt-1">
                ...
                <p class="text-[9px] font-mono text-[#606060]">{{ formatarData((d.maisRecente as any).datanasc) }}</p>
              </div>
            </fieldset>
          </div>
              ...
              <table class="w-full border-collapse">
                ...
                <tbody>
                  <tr v-for="(p, i) in (d.maisConectados as any[])" :key="i"
                    class="border-b border-[#d8d8d8]"
                    :style="{ backgroundColor: p.sexo === 'M' ? 'var(--color-male)' : p.sexo === 'F' ? 'var(--color-female)' : '' }">
                    <td class="py-1 px-1.5 text-[9px] font-mono text-[#606060]">{{ i + 1 }}</td>
        ...
        <div class="grid grid-cols-3 gap-2">
          ...
          <!-- Top 10 sobrenomes -->
          <fieldset class="border-2 border-t-white border-l-white border-r-base-300 border-b-base-300 bg-base-100 px-3 pt-1 pb-2 flex flex-col" style="max-height:220px">
            <legend class="text-[9px] font-bold px-1 uppercase text-[#404040]">Top 10 Sobrenomes</legend>
            <div class="flex-1 overflow-y-auto mt-1 flex flex-col gap-1">
              <div v-for="(s, i) in (d.sobrenomes as any[])" :key="(s as any).sobrenome" class="flex flex-col gap-0.5">
                ...
                <div class="h-1.5 bg-white border border-base-300 overflow-hidden">
                  <div class="h-full bg-[#000080]"
                    :style="{ width: Math.round((s.total / (d.sobrenomes as any[])[0].total) * 100) + '%' }" />
                </div>
              </div>
            </div>
          </fieldset>
          ...
          <fieldset class="border-2 border-t-white border-l-white border-r-base-300 border-b-base-300 bg-base-100 px-3 pt-1 pb-2 flex flex-col" style="max-height:220px">
            <legend class="text-[9px] font-bold px-1 uppercase text-[#404040]">Cidades — Nascimentos</legend>
            <div class="flex-1 overflow-y-auto mt-1 flex flex-col gap-1">
              <div v-if="!(d.cidadesNasc as any[]).length" class="text-[10px] text-[#909090] italic py-2 text-center">Sem dados</div>
              <div v-for="(c, i) in (d.cidadesNasc as any[])" :key="i" class="flex flex-col gap-0.5">
                <div class="flex justify-between items-center">
                  <span class="text-[10px] font-bold truncate max-w-[70%]">{{ c.descricao }}<span v-if="c.estado" class="font-normal text-[#707070]"> / {{ c.estado }}</span></span>
                  <span class="text-[10px] font-mono font-bold">{{ c.total }}</span>
                </div>
                <div class="h-1.5 bg-white border border-base-300 overflow-hidden">
                  <div class="h-full bg-[#008080]"
                    :style="{ width: Math.round((c.total / (d.cidadesNasc as any[])[0].total) * 100) + '%' }" />
                </div>
              </div>
            </div>
          </fieldset>
          ...
          <!-- Cidades de óbito -->
          <fieldset class="border-2 border-t-white border-l-white border-r-base-300 border-b-base-300 bg-base-100 px-3 pt-1 pb-2 flex flex-col" style="max-height:220px">
            <legend class="text-[9px] font-bold px-1 uppercase text-[#404040]">Cidades — Óbitos</legend>
            <div class="flex-1 overflow-y-auto mt-1 flex flex-col gap-1">
              <div v-if="!(d.cidadesObito as any[]).length" class="text-[10px] text-[#909090] italic py-2 text-center">Sem dados</div>
              <div v-for="(c, i) in (d.cidadesObito as any[])" :key="i" class="flex flex-col gap-0.5">
                ...
                <div class="h-1.5 bg-white border border-base-300 overflow-hidden">
                  <div class="h-full bg-[#800000]"
                    :style="{ width: Math.round((c.total / (d.cidadesObito as any[])[0].total) * 100) + '%' }" />
                </div>
              </div>
            </div>
          </fieldset>
    ...
    <!-- Barra de status -->
    <div class="flex-none flex items-center gap-4 px-2 pb-1.5 pt-1 bg-[#c0c0c0] border-t border-base-300">
